              ...
              <div key={m.id} className="space-y-2">
                ...
                {m.pergunta && (
                  <div className="space-y-2 rounded-lg border border-accent/40 bg-card p-2.5">
                    ...
                    <div>
                      <p className="mb-1 text-[10px] text-muted-foreground">{m.pergunta.sliderLabel}</p>
                      <Slider defaultValue={[45]} max={100} step={1} />
                    </div>
                    <button className="flex h-6 w-full items-center justify-center gap-1 rounded border border-dashed border-border text-[11px] text-muted-foreground hover:text-foreground">
                ...
                {m.arquivo && (
                  <div className="flex items-center gap-2 rounded-lg border border-border bg-card p-2.5">
                    ...
                    <button
                      onClick={() => e.abrirDesign("d1")}
                      className="h-6 rounded bg-primary px-2 text-[11px] text-primary-foreground"
                    >
                      Abrir
                    </button>
                    <button className="grid size-6 place-items-center rounded border border-border hover:bg-secondary">
      ...
      <div className="shrink-0 border-t border-border p-2.5">
        ...
        <div className="rounded-lg border border-border bg-card p-2 focus-within:ring-1 focus-within:ring-ring">
          <textarea
            ...
            placeholder="Peça algo, cole uma imagem ou arraste um arquivo…"
          ...
          <div className="flex items-center gap-1">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start">
                ...
                {["arquivo", "imagem", "design system", "repositório", "pasta local"].map((t) => (
                  <DropdownMenuItem key={t} onSelect={() => e.adicionarContexto("Novo item", t)}>
                    {t}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuContent>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start">
                ...
                {["deck", "documento", "protótipo", "vídeo", "PDF"].map((t) => (
                  <DropdownMenuItem key={t}>{t}</DropdownMenuItem>
                ))}
              </DropdownMenuContent>
            </DropdownMenu>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start">
                ...
                {["Rápido", "Equilibrado", "Profundo"].map((t) => (
                  <DropdownMenuItem key={t} onSelect={() => setModelo(t)}>
                    {t}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuContent>
